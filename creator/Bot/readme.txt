...
- config.json - содержит текст для счетчика ЧВ, различный в АН и АА. "Юбилей 'чистого времени' (трезвости, свадьбы, покупки авто)",
  а также смещение времени в минутах от UTC. Тут же можно включить группировку кнопок по 2 в ряд, по условию длины имени
  кнопки.
...
docker run --name name_bot -v ДОМ/РЕГИОН:/home/pi/РЕГИОН:rw --restart=unless-stopped -d -e "CURRENT_DIR=/home/pi/РЕГИОН/БОТ" --user "$(id -u):$(id -g)" kawadiyk/creatorbot:latest ./creator_bot
...
-e "COMMUNITY_TEXT=чистого времени или трезвости или появления питомца или чего угодно" \
-e "TIMEZONE_MINUTES=+180" \
--user "$(id -u):$(id -g)" kawadiyk/creatorbot:latest ./creator_bot
