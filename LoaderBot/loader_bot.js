...
try{config = JSON.parse(fs.readFileSync(currentDir+"/config.json"));
	area = config.area; timePablic = config.timePablic; forDate = config.forDate;
}catch(err){WriteFileJson(currentDir+"/config.json",{"area":area, "timePablic":timePablic, "forDate":forDate});}
...
try{
	...
	{	try {UserList = JSON.parse(fs.readFileSync(FileUserList));} catch(err){console.log(err);}
		...
		let keys = Object.keys(UserList);
        for(let i in keys) str += keys[i]+' : '+UserList[keys[i]]+'\n';
		sendMessage(chatId, str);
	}
	else sendMessage(chatId, smilik);
...
try{
    ...
    {   
		...
		group = group.replace(/'/g, '');
		UserList[id] = name+", гр.<"+group+">";//добавляем новичка
...
{
try{
	fs.writeFile(path, JSON.stringify(arr,null,2), (err) => 
