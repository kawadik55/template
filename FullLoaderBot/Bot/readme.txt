...
Скрипт рассылки rassilka.js переехал внутрь загрузчика и был удален из репозитория.
...
Загруженный контент попадает на модерацию к Админу бота, затем оформляется к публикации ботом из news_bot.json, 
...
Для публикации постов используется бот, прописанный в news_bot.json. Этот бот нужно добавить в канал/группу/личный чат.
...
"chat_news" - списки чат-айди каналов, по группам с локальным смещением времени.
