...
const chat_id_MSO = require(TokenDir+"/chatId.json").chat_id_MSO;//chatId чата МСО
...
if(flagMso && !!chat_id_MSO) MsoBot = new TelegramBot(require(TokenDir+'/'+objbot.file_mso_bot).token, {polling: true});
...
masComi = ['МСО','Исполком','БУ','ПМ','РС','ПСО','МДА','ТЛ','Digital','ЛитКом','Казначей'];
...
WriteLogFile('запуск процесса arch_bot.js', 'непосылать');
...
else {password2 = '222111'; PasswordList.password2=password2; WriteFileJson(FilePasswordList,PasswordList);}//левый, пока нету легального
...
//if(fs.existsSync('/home/pi/context')) WriteLogFile('Содержание:\n'+JSON.stringify(fs.readdirSync('/home/pi/context'),null,2));
//====================================================================
...
try{
	...
	str+='С моей помощью Вы сможете сохранить или прочитать отчеты NAших структур обслуживания. ';
	...
	{	
	    ...
	    {	if(button=='13')//назад
			...
			{	//любая кнопка с месяцем
				...
				str += 'Выбери структуру';
				await sendMessage(chatId, str, EditComiButton(chatId));
			}	
		}
	    ...
	    {	if(button=='01')//записать
			{	if(!(validWriter(chatId) || validAdminBot(chatId)))//если не писатель и не админ
				{	await sendMessage(chatId, 'Извините, ' + name + ', но для записи Вам необходимо сначала пройти авторизацию!');
					send_instruction(chatId,user,'');
					return;
				}
...
{	process.on(event, async ()=>
	{	//await WriteFileJson(FilePID,'{}');
		...
		{if(err) console.log(err);
		 await WriteLogFile('выход из процесса arch_bot.js', 'непосылать');
		 process.exit();
		});
	});
});
