...
arch_bot.js - основной скрипт бота @na_kzn_arch_bot.
...
МСО и ловит сообщения с нужными тегами. Этот бот должен быть создан с настройкой Bot Setting -> Group Privacy = off, 
и добавлен в телеграм-группу МСО.
В этой группе должны состоять служащие подкомитетов, которые будут публиковать в этой группе отчеты. Для того, чтобы отчет автоматически
сохранился в базе, в начале текста необходимо разместить хештег = #отчет ЛитКом-02-2025 и далее через пробел или с новой строки
текст самого отчета. В хештеге 'ЛитКом' - аббревиатура подкомитета, '02' - отчетный месяц, '2025' - отчетный год. Аббревиатура
подкомитета должна обязательно совпадать с одним из шаблонов в файле comitee.txt. Тоже самое справедливо и для
...
Готовый образ для Докера "kawadiyk/archbot:latest" можно загрузить из DockerHub.
//-----------------------------------------------------------------------------------
...
-e "TOKEN_MSO=токен для бота слушателя в чате МСО, если нужен" \
