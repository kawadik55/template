...
const { Bot } = require('@kawadik55/my-max-bot-api');
...
var DirBaseES = currentDir+'/../Rassilka/BaseES';//папка с расписанием от РЗФ
var FileNameRaspisES = 'raspisES.html';//имя файла с расписанием от РЗФ
const smilik = '¯\\_(ツ)_/¯';
...
try{config = JSON.parse(fs.readFileSync(currentDir+"/config.json"));
	...
	if(!config.fromES) {config.fromES = false; WriteFileJson(currentDir+"/config.json",config);}
	if(!config.DirBaseES) {config.DirBaseES = "/../Rassilka/BaseES"; WriteFileJson(currentDir+"/config.json",config);}
	if(!config.FileNameRaspisES) {config.FileNameRaspisES = "raspisES.html"; WriteFileJson(currentDir+"/config.json",config);}
}catch(err)
{config = {"area":area, "timePablic":timePablic, "utcOffset":String(utcOffset), "forDate":forDate, 
		"lifeTime":lifeTime, "rassilka":rassilka, "hostingImg":hostingImg, "pathHostingImg":"/../www/img", 
		"hostname":"https://vps.na-server.ru", "Supervisor":"1234567", "queuelimit":200, "slavebot":false, 
		"fromES":false, "DirBaseES":"/../Rassilka/BaseES", "FileNameRaspisES":"raspisES.html"
		};
...
if(DirBaseES.endsWith('/')) DirBaseES = DirBaseES.slice(0, -1);
FileNameRaspisES = config.FileNameRaspisES;//имя файла в Расписаниях ЕС
if (FileNameRaspisES.startsWith('/')) FileNameRaspisES = FileNameRaspisES.substring(1);
...
{	if(rassilka && config.News===true)//если рассылка включена
	{	let now = moment();
		...
		//приводим к 0 сек
		now = now.subtract(10, 'seconds');
		//публикуем тексты и фото в ТГ
...
async function publicText(obj,offset)//тут текст на публикацию в ТГ
{
try{
...
try{
    ...
    {	let timestr = obj?.time || '';//запись времени
		...
		if(obj.media)
		{	data.paths = obj.media.map(item => item.media);
		}
...
{ try
  {		if(!queueWebMax && !queueBotMax) return;
	...
	async function go2public(chat,groffset,napr)
	{
		...
		for(let i=0;i<chat.length;i++) 
		{  try{	
			if(!chat[i].Raspis) continue;//не выбран Расписание в доставке
			let chatId = chat[i].chatId ? chat[i].chatId : '';
			let name = chat[i].name ? chat[i].name : 'undefined';
			if(!chatId) continue;//пропускаем цикл, если нет chatId
			//отправляем в очередь
  ...
  {	if(!queueWebMax && !queueBotMax) return;
	...
	async function go2public(chat,groffset,listTown,napr)
	{
		...
		{  try{
			...
			let name = chat[i].name ? chat[i].name : 'undefined';
			if(!chatId) continue;//пропускаем цикл, если нет chatId
			let town = chat[i].town ? chat[i].town : 'не выбран ('+name+')';
			//запросим объект расписания в городе
			let obj = await getRaspisBaseES(chat[i], groffset, listTown);
...
{ try
  {	//ищем файл со списком городов listTowns.json
	...
	for(let i=0;i<offset.length;i++)
	{	let userHour = getUserDateTime(now, Number(offset[i])).hour();//час юзера как число
		if(userHour===publicHour) await go2public(chat_news[offset[i]],offset[i],listTowns);//передаем массив чатов
	}
  ...
  {WriteLogFile(err+'\nfrom send_Raspis_ES()','вчат');
  }
}
//====================================================================
async function getRaspisBaseES(chatobj, offset, listTowns, format)
{	
try{
	//загрузим город чата
	...
	{	//ищем в списке город по slug
		if(!!slug && slug !== true) town = Object.keys(listTowns).find(key => listTowns[key].slug === slug);
	}
	if(!town) 
	{	WriteLogFile('Отсутствует town='+chatobj.town+' в списке городов\n getRaspisBaseES('+chatobj.town+')','вчат');
	...
	{	WriteLogFile('Ошибка в основном файле расписания\n getRaspisBaseES('+slug+')','вчат');
		return null;//'Сожалею, но я не могу прочитать расписание в файле... 😥';
	}
	
	//дата у юзера
	const userDate = getUserDateTime(moment(),offset).startOf('day');
	//дата Расписания на сегодня из UnixTime
	const todayDate = raspis.momentTime ? moment(raspis.momentTime).startOf('day') : moment().startOf('day');
	//разница в днях
	...
	{	if(diffDays > 0)
		...
		try{raspis = JSON.parse(fs.readFileSync(refpath));}catch(err){console.log(err);}
		if(!raspis || (typeof(raspis) !== 'object') || Object.keys(raspis).length===0)
		{	WriteLogFile('Ошибка в сдвинутом файле расписания\n getRaspisBaseES('+slug+')','вчат');
			return null;//'Сожалею, но я не могу прочитать расписание в файле... 😥';
		}
	}
	...
	{   try{  
          ...
          { let timestr = !!TextList[key].time?(' '+TextList[key].time):'';
			...
			{	let chatId = '';
				...
				if(res===false) WriteLogFile('Не смог послать в ТГ текст text "'+key+'"'+' в '+name[0]);
				else if(Object.hasOwn(res, 'code'))//в ответе есть ошибка
				{	
					if(res.code.indexOf('ETELEGRAM')+1)//ошибка от Телеги 
					{//нельзя послать сообщение админу в телегу
...
{	offset = Number(offset);
	let reftime = now ? moment(now) : moment();
	const ms = reftime.millisecond();
	let userTime = (reftime.unix() + ((offset - utcOffset) * 60)) * 1000 + ms;//в мсек
	return moment(userTime);//дата/время юзера
}
//====================================================================
...
{	let now = moment();//по-умолчанию
	...
	{	try {const obj = JSON.parse(fs.readFileSync(timestampPath));
			if (obj.momentTime) now = moment(obj.momentTime);//дата/время создания файла Ежика
			else if(!!obj.UnixTime) now = moment.unix(obj.UnixTime);//дата/время создания файла Ежика
		} 
		catch (err) {console.log(err);}
	}
...
{	//ищем город по имени или по слагу
	...
	let towns = Object.keys(listTowns);
	if(towns.length===0) return [];
	name = name.toLowerCase().trim();
	let res = towns.filter(town => town.toLowerCase().includes(name));
	if(res.length===0) return [];
	let result = [];
	for(let i in res) result.push({town:res[i], slug:listTowns[res[i]].slug});
	result.sort((a, b) => a.town.length - b.town.length);//сортируем по длине
	return result;
}
//====================================================================
