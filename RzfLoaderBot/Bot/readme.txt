...
"slavebot":true (для ТГ). Если его выключить (false), то все настройки на канал/группу/личный чат админу нужно будет прописывать
...
Для Макса работает свой slavebot, который включается в поле "useBotMax". Принципиально - это копия слэйв-бота ТГ, только для Макса.
...
Также добавлена папка Max с модулями для публикаций в Максе через бота и по Web сокету.

Служебные файлы:
- /images - папка для хранения загруженных картинок.
- /moder - папка для ожидающих модерацию картинок.
- /Token - папка с токенами ботов ТГ и Макса.
- /Max - папка с модулями для Макса.
...
"timePablic" - время утренней публикации постов.
...
"DirBaseES" - относительный путь к папке с расписаниями РЗФ.
"fromES" - переключатель true/false, если true - используется функционал расписания с ES, иначе - обычное расписание местности.
"DirBaseES": относительный путь к базе BaseES с расписаниями всех городов с Единого Сайта РЗФ.
"FileNameRaspisES": "raspisES.html",
"useWebMax" - переключатель true/false для включения рассылки в Макс через Web сокет.
"useBotMax" - переключатель true/false для включения рассылки в Макс через бот, если он есть. Принцип работы как у slavebot.
...
Файл для токена пользователя в Макс max_web.json создается автоматически, если его не было. При использовании рассылки
в Макс, его тоже нужно заполнить необходимыми данными.
