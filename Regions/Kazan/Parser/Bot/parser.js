...
//const token = require(TokenDir+"/logs_bot.json").token;//бот @na_kzn_news_bot
    ...
    if(response.statusCode==200)
    {
     ...
     { //пройдемся по дням недели
       ...
       {  var mas = tables[j].getElementsByClassName('rasp__title');//день недели
		  ...
		  week[day] = [];//создаем массив дня недели
...
{      //запишем текст текущего дня в файл в формате HTML
       ...
       //let gr = Object.keys(mas_week[masDay[dayWeek]]);//массив названий групп в этот день
	   ...
	   {	//сортируем группы по времени
			...
			{ 	let url;
				let name = gr[j].name;
				//сделаем гиперссылку
				if(!!gr[j].url) url = '<a  href="'+gr[j].url+'" >'+name+'</a>';
