...
const ParserDir=currentDir+"/../Parser";//путь к папке с токенами, на уровень выше
...
//const FileEg = 		ParserDir+'/eg.txt';//файл с ежиком
//const FileRaspis = ParserDir+'/raspis.txt';//файл с расписанием на день
...
//chat_news[0] = require(TokenDir+"/chatId.json").na_ufa_news;
//---------------------------------------------------
  ...
  {	if(flagRaspis)
	{
		...
		const keyboard = getButtonUrl(mode,true);//прилепим кнопку с ботом, отключим превью ссылок
...
function getButtonUrl(pars, preview)
{
	...
	{ 	reply_markup: 
		{ inline_keyboard: 
			[ [{"text": "Бот АН Уфа", "url": "https://t.me/na_ufa_bot"}]
			]	 
		}
	};
