...
//chat_news[0] = require(TokenDir+"/chatId.json").na_ufa_info;
//---------------------------------------------------
  ...
  {	if(fun['sendImages']) {clearTimeout(fun['sendImages']); fun['sendImages'] = null;}
	...
	{	  let flag = 0;
          ...
          { 
			let opt = {};
			let caption = '';
			if(Object.hasOwn(ImagesList[key], 'caption')) caption = ImagesList[key].caption;
			if(caption.indexOf('#кнопкабота')+1)//если есть хештег кнопки 
			{	caption = caption.replace('#кнопкабота','');//убираем хештег
				opt = getButtonUrl('',true);//кнопка с ботом, без превью в картинках
			}
			//текст подписи
...
{ try
  {	
	...
	{     let date = TextList[key].date;//запись даты
		  ...
		  if(flag)
          { let opt = {};
            let text = TextList[key].text;
			if(text.indexOf('#кнопкабота')+1)//если есть хештег кнопки 
			{	text = text.replace('#кнопкабота','');//убираем хештег
				opt = getButtonUrl('',false);//кнопка с ботом, c превью
			}
			//форматирование
            ...
            for(let i=0;i<chat_news.length;i++) 
			{	let res = await sendTextToBot(chat_news[i], text, opt);
				//console.log('res='+JSON.stringify(res,null,2));
				if(Object.hasOwn(res, 'code'))//в ответе есть ошибка
				{	
					if(res.code.indexOf('ETELEGRAM')+1)//ошибка от Телеги 
