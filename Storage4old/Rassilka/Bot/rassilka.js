          ...
          { let opt = new Object();
			...
			{	let res;
				...
				{	good++;//если без ошибок
					console.log(getTimeStr()+' "'+key+'"'+' в '+name[i]+' = ОК');
				}
			}
          }
...
{ try
  {	if(flagEg)
	{
		if(!eg) {console.log(getTimeStr()+'файл с ежиком отсутствует'); return;}
		...
		{  try{	
			...
			{	good++;//если без ошибок
				console.log(getTimeStr()+'в '+name[i]+' = ОК');
			}
		  }catch(err){WriteLogFile(err+'\nfrom send_Eg()=>for()');}
		}
	}
...
{ try
  {	if(flagRaspis)
	{
		if(!raspis) {console.log(getTimeStr()+'файл с расписанием отсутствует'); return;}
